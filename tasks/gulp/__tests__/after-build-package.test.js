...
describe('package/', () => {
  ...
  describe('README.md', () => {
    it('is not overwritten', () => {
      return readFile(path.join(configPaths.package, 'README.md'), 'utf8')
        .then(contents => {
          // Look for H1 matching 'London Borough of Sutton (LBS) Frontend' from existing README
          expect(contents).toMatch(/^# London Borough of Sutton \(LBS\) Frontend/)
        }).catch(error => {
          throw error
        })
    })
  ...
  describe('all.scss', () => {
    it('should compile without throwing an exception', async () => {
      const allScssFile = path.join(configPaths.package, 'LBS', 'all.scss')
  ...
  describe('component', () => {
    it.each(componentNames)('\'%s\' should have macro-options.json that contains JSON', (name) => {
      const filePath = path.join(configPaths.package, 'LBS', 'components', name, 'macro-options.json')
  ...
  describe('fixtures', () => {
    it.each(componentNames)('\'%s\' should have fixtures.json that contains JSON', (name) => {
      const filePath = path.join(configPaths.package, 'LBS', 'components', name, 'fixtures.json')
      return readFile(filePath, 'utf8')
        .then((data) => {
          var parsedData = JSON.parse(data)
