...
describe('dist/', () => {
        ...
        return recursive(path.join(configPaths.src, 'assets'), filesToIgnore).then(
          files => {
            return files
              ...
              .map(file => file.replace(/^src\/LBS\/assets\//, ''))
  ...
  describe(`lbs-frontend-${version}.min.css`, () => {
    const stylesheet = lib.readFileContents(path.join(configPaths.dist, `lbs-frontend-${version}.min.css`))
  ...
  describe(`lbs-frontend-${version}.min.js`, () => {
    const javascript = lib.readFileContents(path.join(configPaths.dist, `lbs-frontend-${version}.min.js`))
